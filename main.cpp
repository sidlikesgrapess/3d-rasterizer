...
public:
    ...
    while (getline(file, line)) {
      ...
      if (type == "v") {
        ...
        for (size_t i = 1; i + 1 < face_v.size(); ++i) {
          mesh.tris.push_back({mesh.vertices[face_v[0]],
                               mesh.vertices[face_v[i]],
                               mesh.vertices[face_v[i + 1]]});
        }
      }
    }
    return mesh;
  ...
  bool OnUserUpdate(float fElapsedTime) {
    ...
    for (const auto &tri : cube.tris) {
      Triangle transformed = translate_z_triangle(
          rotate_yz_triangle(rotate_xz_triangle(tri, angle), angle2), z_offset);
  ...
  void Fill_flat_bottom_triangle(Vec2D p0, Vec2D p1, Vec2D p2, Color color) {
    ...
    for (int y = startY; y <= endY; y++) {
      ...
      DrawLine(x1, (float)y, x2, (float)y, color);
    }
  }
